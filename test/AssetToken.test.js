...
contract("AssetToken", ([deployer, admin, account1, account2]) => {
  ...
  context("AssetToken.new", () => {
    it("has given data", async () => {
      ...
      expect(await assetToken.getElPrice()).to.be.bignumber.equal(
        new BN(elPrice)
      );
      expect(await assetToken.getRewardPerBlock()).to.be.bignumber.equal(
  ...
  context("AssetToken is deployed", () => {
    ...
    describe(".purchase", async () => {
      it("if account has sufficient allowed el balance, can purchase token", async () => {
        await el.transfer(account1, to18DecimalBN(10000).toString(), {
          from: deployer,
        });
        await el.approve(assetToken.address, elTotalSupplyString, {
          from: account1,
        });

        await assetToken.purchase(20, { from: account1 });

        expect(await assetToken.balanceOf(account1)).to.be.bignumber.equal(
          new BN(20)
        );
    ...
    describe(".refund", async () => {
      it("if account and contract has sufficient balance, refund token", async () => {
        ...
        await el.transfer(assetToken.address, to18DecimalBN(10000).toString(), {
          from: deployer,
        });
        await el.approve(assetToken.address, elTotalSupplyString, {
          from: account1,
        ...
        expect(
          (await el.balanceOf(assetToken.address))
            .sub(to18DecimalBN(10000))
            .add(await assetToken.toElAmount(10))
        ).to.be.bignumber.gte(new BN(1));

        expect(
          (await el.balanceOf(account1))
            .sub(to18DecimalBN(10000))
            .sub(await assetToken.toElAmount(10))
      ...
      it("if account does not have sufficient balance, transfer is failed", async () => {
        await assetToken.approve(assetToken.address, totalSupply, {
          from: account1,
        });
    ...
    describe(".claimReward", async () => {
      beforeEach(async () => {
        ...
        await el.transfer(account2, await el.balanceOf(account1), {
          from: account1,
        });

        this.secondBlock = (
          await assetToken.transfer(account2, 10, { from: account1 })
        ).receipt.blockNumber;
        this.thirdBlock = (
          await assetToken.transfer(account2, 10, { from: account1 })
        ).receipt.blockNumber;
      ...
      it("whitelisted account can claim reward.", async () => {
        ...
        expect(await assetToken.getReward(account1)).to.be.bignumber.equal(
          new BN(0)
        );
      });

      it("un whitelisted account cannot claim reward.", async () => {
        expect(await assetToken.getReward(account2)).not.to.be.bignumber.equal(
          new BN(0)
        );
    ...
    describe(".withdrawElToAdmin", async () => {
      ...
      it("admin can withdrwal all el.", async () => {
        ...
        expect(await el.balanceOf(admin)).to.be.bignumber.equal(
          new BN(to18DecimalBN(10000))
        );
      });
