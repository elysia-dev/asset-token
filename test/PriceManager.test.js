...
contract("PriceManager", (accounts) => {
  ...
  beforeEach(async () => {
    priceManager = await PriceManagerTest.new(
      "name",
      "symbol",
      0,
      10000,
  ...
  context(".setElPrice", async () => {
    it("Admin can set ElPrice", async () => {
      const setElPriceTx = await priceManager.setElPrice(elPrice, {
        from: admin,
      });
  ...
  context(".setPrice", async () => {
    it("Admin can set price", async () => {
      ...
      expect(await priceManager.getPrice()).to.be.bignumber.equal(
        new BN(price)
      );
