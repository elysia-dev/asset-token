...
contract('RewardManager', (accounts) => {
    ...
    const account1RewardPerBlock =  new BN(rewardPerBlock).mul(new BN(10)).div(new BN(totalSupply))
    ...
    describe('.setRewardPerBlock', async () => {
        it('admin can set rewardPerBlock', async () => {
            const tx = await rewardManager.setRewardPerBlock(rewardPerBlock, {from: admin});

            expect(await rewardManager.getRewardPerBlock()).to.be.bignumber.equal(new BN(rewardPerBlock));

            expectEvent(tx, 'NewRewardPerBlock', {newRewardPerBlock: rewardPerBlock});
        })
    ...
    context('when rewardPerBlock is set', async () => {
        ...
        describe('.getReward', async () => {
            ...
            it('if account has token, return increased reward with blockNumbers', async () =>{
                const txBefore = await rewardManager.transfer(account2, 10, {from: admin});
                const beforeReward = await rewardManager.getReward(account1)
                const txAfter = await rewardManager.transfer(account2, 10, {from: admin});
                ...
                expect(afterReward.sub(beforeReward)).to.be.bignumber.equal(
                    ...
                    .mul(
                        new BN(
                            (txAfter.receipt.blockNumber - txBefore.receipt.blockNumber)
                        )
                    )
                );
            })
            ...
            it('if account do not have token, return zero', async () =>{
                ...
                expect(beforeReward).to.be.bignumber.equal(
                    new BN(0)
                );

                expect(afterReward).to.be.bignumber.equal(
                    new BN(0)
                );
            })
        })
        ...
        describe('.saveReward', async () => {
            it('if account has token, reward is saved', async () =>{
                ...
                expect(await rewardManager.getReward(account1)).to.be.bignumber.equal(
                    ...
                    .mul(
                        new BN(
                            (tx2.receipt.blockNumber - tx1.receipt.blockNumber)
                        )
                    )
                );
            })
            ...
            it('if user has no token, save zero value', async () =>{
                ...
                expect(await rewardManager.getReward(account2)).to.be.bignumber.equal(new BN(0));
            })
        })
